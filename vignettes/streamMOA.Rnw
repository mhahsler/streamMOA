...
sapply(
  ...
  FUN = function(a) {
    ...
    evaluate_static(
      a,
      stream,
      measure = c("numMacroClusters", "purity", "SSQ", "cRand"),
      n = 500,
      assign = "micro",
      type = "macro"
    )
...
algorithms <- list(
  'Sample' = DSC_TwoStage(
    micro = DSC_Sample(k = 100, biased = TRUE),
    macro = DSC_Kmeans(k = 2)),
  'Window' = DSC_TwoStage(
    micro = DSC_Window(horizon = 101, lambda = .01),
    macro = DSC_Kmeans(k = 2)
  ),
  'DBSTREAM' = DSC_DBSTREAM(r = .5, lambda = .01),
...
We perform the evaluation using \code{evaluate_stream} which
